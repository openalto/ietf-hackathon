...
ATLAS [[2]](#2) experiment is one the four major experiments at the Large Hadron Collider (LHC) [[3]](#3) at CERN [[4]](#4). Atlas generates more than one petabyte of data every day [[5]](#5). Atlas computing and storage needs are unprecedentedly heterogenous: In addition to massive volume, data generation, storage, access, and processing locations are divergent.

Rucio is built on top of Atlas storage systems to scalably and efficiently manage the replication and archiving of the created data and handle access to data. "One of the guiding principles of Rucio is data flow autonomy and automation": As a result, Rucio provides a unified interface for accessing and controlling the underlying diversified networking and storage services (such as XRootD and S3). Scientists and admins manage data replication and access with Rucio.  

Rucio collects various information about the network and storage devices that guide data identifier transfers between sites and source selections before downloads [[8]](#8). Most importantly, distances between RSEs (Rucio Storage Elements), according to the aggregated download throughput between RSEs or geographical location databases [[1]](#1). However, such metrics are static and non-informative.

ALTO (Application-Layer Traffic Optimization) framework collects network information and exposes it to applications to steer traffic. ALTO is currently used in multiple large-scale systems.

At IETF 113 Hackathon event, ALTO is used to provide Rucio's replication sorting and transfer scheduling parts with more accurate information about the network state. [Evaluation results](#Evaluations) showed that Alto-based replica sorting decreases replica transfer times by up to x times. 
